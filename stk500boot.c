...
// Enable the use of a switch connected to a pin to hold the bootloader in 
// wait for firmware upload mode
#define USE_PROG_PIN
#ifdef USE_PROG_PIN
	#define PROG_PIN_DDR   	DDRK
	#define PROG_PIN_PORT  	PORTK
	#define PROG_PIN 				PINK
	#define PROG_PIN_BIT   	2	
	int holdUpBootloader = 0;  // a flag that holds us in mode after the button is released
#endif
...
#ifdef USE_PROG_PIN
	PROG_PIN_DDR   = 0x00; // Set port to all inputs
	PROG_PIN_PORT = 0xFF; // Turn on all pull ups
#endif
...
//	sendchar('0');
	...
	while (boot_state==0)
	{
		while ((!(Serial_Available())) && (boot_state == 0))		// wait for data
		{
			...
			#ifdef USE_PROG_PIN
			  /*
				The Programming pin is an input that is pulled high.  In the open switch
				state we will see a 1 and when the switch closes (pushed) to ground we will see a 0
				If we see a zero we will reset the boot_timer and set a flag.  This gets a stuck in the
				mode when firmware can be uploaded.
        Summary: To get in the this reboot with the button pushed				
				*/
			  if (((PROG_PIN & (1 << PROG_PIN_BIT)) == 0) | holdUpBootloader)
				{		
				   boot_timer = 0;  // reset the counter
					 holdUpBootloader = 1;				   
				} 
				else
